...
function CartProvider({
  ...
  const mergedConfig = (0, import_lodash_es.merge)({}, defaultConfig, config);
  const [typedCartAttributes, setTypedCartAttributes] = (0, import_react.useState)({});
  ...
  const addProducts = async (lines) => {
    ...
    try {
      ...
      for (const incomingLine of lines) {
        ...
        if (existing) {
          const newQty = existing.quantity + (incomingLine.quantity || 1);
          debug && log(
            `[CartContext] Updating quantity of ${incomingLine.merchandiseId} to ${newQty}`
          );
          updates.push(client.updateCartItem(cart.id, existing.id, newQty));
        } else {
          debug && log(`[CartContext] Adding new item ${incomingLine.merchandiseId}`);
          updates.push(client.addToCart(cart.id, [incomingLine], mergedConfig));
        }
      }
      const results = await Promise.all(updates);
      const finalCart = results.length > 0 ? results[results.length - 1] : cart;
